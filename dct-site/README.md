...
- Google Ads has exactly two reusable account-level negative-keyword lists, both attached to the current DCT campaign: `DCT | Global Country Negatives` (176 broad-match non-approved country/destination terms, including Canada, USA and United States) and `DCT | Global Search Exclusions` (the campaign's 71 job, support, irrelevant-brand and other search exclusions). Kiran's approved destinations are excluded from the country list. On 10 September 2026, all 28 live positive keyword criteria containing Canada or Morocco were removed, and the campaign builder no longer recreates them. The campaign's Canada geographic audience targeting remains in place so Canadian customers can still find the approved European-tour offers. The consolidation script is `tools/consolidate_google_ads_negatives.py`; it must be used for future negative-list changes rather than adding loose campaign criteria.
- The search campaign runs six ad groups: one per operator (`DCT | Trafalgar`, `DCT | Insight`, `DCT | Globus`, `DCT | Cosmos`), plus `DCT | Brand` and `DCT | Generic Coach Tours | HOLD`. On 10 September 2026 the paired `Brand & Tours` / `Destinations` groups were merged one-per-operator to stop an already-thin conversion signal being split eight ways on a C$65/day budget. Each merged group holds both keyword sets and both RSAs. The four `| Destinations` groups were PAUSED, not removed, so the merge is reversible. The tool is `tools/consolidate_google_ads_ad_groups.py` (`--plan` / `--verify` are read-only, `--apply` mutates).
- While copying the destination RSAs into the merged groups, ad copy naming destinations RCN does not sell was replaced with approved ones: Morocco headlines and description mentions on Trafalgar and Insight, and 'Canada Tours' headlines and description mentions on Globus and Cosmos. Those assets predated the destination cleanup and were still serving. The originals survive in the paused groups.
- `tools/build_google_ads_campaign.py` no longer matches the live account: it still emits the ten-ad-group paired structure and creates no negative keywords. Treat it as a historical build record, not a rebuild path. Rebuilding from it would need the ad group structure updating and shared-set attachment adding first.
